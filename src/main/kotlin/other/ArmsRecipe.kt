...
import other.src.app_package.armsActivity
import other.src.app_package.armsFragment
import other.src.app_package.armsRepo
import other.src.app_package.armsViewModel
...
fun RecipeExecutor.armsRecipe(provider: ArmsPluginTemplateProviderImpl, data: ModuleTemplateData) {
    ...
    if (provider.needViewModel.value) {
        ...
        save(armsViewModel(isKt, provider), viewmodelFile)
        open(viewmodelFile)
    }

//    if (provider.needRepo.value) {
//        val repoFile = File(data.rootDir, "${fFmSlashedPackageName(provider.repoPackageName.value)}/${provider.pageName.value}Repo.$languageSuffix")
//        save(armsRepo(isKt, provider), repoFile)
//        open(repoFile)
//    }
